...
static int findKeywordInLine(const char *line, const char *keyword, int caseSensitive, int wholeWord) {
    if (!line || !keyword || keyword[0] == '\0') return 0;
    int lineLen = strlen(line);
    int keywordLen = strlen(keyword);
    if (keywordLen == 0 || keywordLen > lineLen || keywordLen > MAX_WORD_LEN) return 0;

    int matchFound = 0;
    const char *pos = line;

    // Цикл завершается, когда найдено совпадение или достигнут конец строки
    while (!matchFound && *pos) {
        ...
        if (caseSensitive) {
            ...
            while (*searchPos && !found) {
                int match = 1;
                for (int i = 0; i < keywordLen && match; i++) {
                    if (searchPos[i] == '\0' || tolower((unsigned char)searchPos[i]) != tolower((unsigned char)keyword[i])) {
                        match = 0;
                    }
                }
                if (match) found = searchPos;
                if (!found) searchPos++;
            }
        }

        if (!found) {
            // Совпадений больше нет, цикл завершится на следующей итерации
        } else if (wholeWord) {
            int isBoundaryStart = (found == line) || isWordBoundary(found[-1]);
            int isBoundaryEnd = (found[keywordLen] == '\0') || isWordBoundary(found[keywordLen]);
            ...
            matchFound = 1; // Найдено без проверки границ слова
        }
    }
    return matchFound;
}
...
void searchInFile(const char *filepath, const SearchParams *params) {
    ...
    // Проверка лимита строк вынесена в условие цикла 
    while (fgets(line, MAX_LINE_LEN, file) && !g_searchCanceled && lineNumber <= 1000000) {
        lineNumber++;
            ...
            if (line[0] != '\0') {
                for (int i = 0; i < params->keywordCount && i < MAX_KEYWORDS; i++) {
                    if (params->keywords[i][0] != '\0') {
                        if (findKeywordInLine(line, params->keywords[i], params->caseSensitive, params->wholeWord)) {
...
void searchInDirectory(const char *path, const SearchParams *params) {
    ...
    if (!params) return;
    if (g_directoryDepth >= MAX_DIRECTORY_DEPTH) return;
    ...
    if (searchPattern[len - 1] != L'\\' && searchPattern[len - 1] != L'/') {
        if (len + 1 >= MAX_PATH_LEN - 3) { g_directoryDepth--; return; }
    ...
    do {
        if (!g_searchCanceled) {
                ...
                if (pathLen + fileNameLen < MAX_PATH_LEN) {
                    ...
                    if (fullPath[0] != '\0') {
                        ...
                        if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                            ...
                            if (matchesExtension(fileName, (char (*)[MAX_EXT_LEN])params->extensions, params->extensionCount)) {
            ...
            shouldContinueSearch = 0;
        }
    } while (shouldContinueSearch && FindNextFileW(hFind, &findData) != 0);
